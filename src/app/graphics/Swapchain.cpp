...
void Swapchain::setup(uint32_t width, uint32_t height){
	...
	if(_imageCount < capabilities.minImageCount){
		Log::error("GPU :Swapchain doesn't allow for %u images.", _imageCount );
		return;
	}
	...
	// The game was not sRGB aware, so use a linear output surface, but in sRGB colorspace.
	const VkColorSpaceKHR tgtColorSpace = VK_COLOR_SPACE_SRGB_NONLINEAR_KHR;
	const std::vector<VkFormat> tgtFormats = { VK_FORMAT_B8G8R8A8_UNORM, VK_FORMAT_R8G8B8A8_UNORM, VK_FORMAT_B8G8R8_UNORM, VK_FORMAT_R8G8B8_UNORM};
	...
	if(!foundTargetSurfaceParams){
		Log::warning( "Could not find a target surface format, using whatever is available. Gamma issues might appear.");
	...
	if(vkCreateSwapchainKHR(_context->device, &swapInfo, nullptr, &_swapchain) != VK_SUCCESS) {
		Log::error("GPU: Unable to create swap chain.");
		return;
	}
	...
	for(uint i = 0; i < _imageCount; ++i){
		...
		Texture& color = _colors.emplace_back("Color");
		...
		VkUtils::setDebugName(*_context, VK_OBJECT_TYPE_IMAGE_VIEW, uint64_t(color.gpu->view), "Swapchain color %u - global", i);
		...
		VkUtils::setDebugName(*_context, VK_OBJECT_TYPE_IMAGE_VIEW, uint64_t(color.gpu->views[0].mipView), "Swapchain color %u - mip0", i);
		VkUtils::setDebugName(*_context, VK_OBJECT_TYPE_IMAGE_VIEW, uint64_t(color.gpu->views[0].views[0]), "Swapchain color %u - mip0 - level0", i);
	}
	...
	for(size_t i = 0; i < _framesFinished.size(); i++) {
		...
		if(availRes != VK_SUCCESS || finishRes != VK_SUCCESS || inflightRes != VK_SUCCESS){
			Log::error("GPU: Unable to create semaphores and fences.");
		}
	}
...
void Swapchain::clean() {
	...
	// We have to manually delete the color textures, because they don't own their color images (system created) nor their depth texture (shared).
