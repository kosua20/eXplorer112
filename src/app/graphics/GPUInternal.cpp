...
VKAPI_ATTR VkBool32 VKAPI_CALL vkDebugCallback(VkDebugUtilsMessageSeverityFlagBitsEXT messageSeverity, VkDebugUtilsMessageTypeFlagsEXT messageType, const VkDebugUtilsMessengerCallbackDataEXT* callbackData, void* userData){
	...
	if(messageSeverity & VK_DEBUG_UTILS_MESSAGE_SEVERITY_ERROR_BIT_EXT){
		...
		Log::warning("GPU: %s", message.c_str());
	} else if(messageSeverity & VK_DEBUG_UTILS_MESSAGE_SEVERITY_INFO_BIT_EXT){
		Log::warning("GPU: %s", message.c_str());
	} else if(messageSeverity & VK_DEBUG_UTILS_MESSAGE_SEVERITY_VERBOSE_BIT_EXT){
		Log::verbose("GPU: %s", message.c_str());
	}
	return VK_FALSE;
}
...
Layout VkUtils::convertFormat(const VkFormat& format){
	static const std::map<VkFormat, Layout> formatInfos = {
		...
		{ VK_FORMAT_BC1_RGBA_UNORM_BLOCK, Layout::BC1 },
		{ VK_FORMAT_BC2_UNORM_BLOCK, Layout::BC2 },
		{ VK_FORMAT_BC3_UNORM_BLOCK, Layout::BC3 }
	};
...
#define VALIDATE_TRANSITIONS
	...
	for(uint mid = mipStart; mid < mipStart + mipCount; ++mid){
		for(uint lid = layerStart; lid < layerStart + layerCount; ++lid){
			...
			if(did == listCount){
				Log::error("GPU: Unsupported transition.");
				continue;
			}
#endif
...
void VkUtils::checkResult(VkResult status){
	...
	Log::error( "Vulkan error code: %s", errorType.c_str());
...
unsigned int VkUtils::getGPULayout(Layout typedFormat, VkFormat & format) {
	...
	static const std::unordered_map<Layout, FormatAndChannels> formatInfos = {
		...
		{Layout::BC1, { VK_FORMAT_BC1_RGBA_UNORM_BLOCK, 4 }},
		{Layout::BC2, { VK_FORMAT_BC2_UNORM_BLOCK, 4 }},
		{Layout::BC3, { VK_FORMAT_BC3_UNORM_BLOCK, 4 }}
	};
	...
	Log::error("GPU: Unable to find type and format (typed format %u).",  uint(typedFormat));
	return 0;
}
...
glm::uvec2 VkUtils::copyTextureRegionToBuffer(VkCommandBuffer& commandBuffer, const Texture & srcTexture, std::shared_ptr<Buffer> & dstBuffer, uint mipStart, uint mipCount, uint layerStart, uint layerCount, const glm::uvec2& offset, const glm::uvec2& size){
	...
	GPU::setupTexture(transferTexture, formats[channels], false);
	...
	for(uint mid = 0; mid < mipCount; ++mid){
		...
		// Number of images.
		const uint imageSize = w * h * sizeof(unsigned char) * channels;
