...
Config::Config(const std::vector<std::string> & argv) {
	...
	if(potentialConfig == "c" || potentialConfig == "config") {
		// Safety check.
		if(argv.size() < 3) {
			Log::error("Config: Missing path for --config argument. Using default config.");
			return;
		}
...
void Config::parseFromFile(const std::string & filePath, std::vector<KeyValues> & arguments) {
	...
	const std::string configContent = System::loadString(filePath);
	if(configContent.empty()) {
		Log::error("Config: Missing/empty config file. Using default config.");
		return;
	}
...
bool Config::showHelp() {
	if(_showHelp) {
		...
		for(size_t aid = 0; aid < _infos.size(); ++aid) {
			...
			Log::info("%s", namesAndValues[aid].c_str());
			for(size_t i = namesAndValues[aid].size(); i < maxSize; ++i) {
				Log::info( " " );
			}
			Log::info( "  %s\n", argInfos.details.c_str());
		}
	}
	return _showHelp;
}
